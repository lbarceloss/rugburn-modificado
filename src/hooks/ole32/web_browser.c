...
#define WEBVIEW2_TEST_URL L"https://www.youtube.com/"
...
static HRESULT STDCALL CoGetClassObjectHook(const IID *rclsid, DWORD dwClsContext,
    ...
    Log("[diag] CoGetClassObject clsid={%08x-%04x-%04x-...}\r\n",
        rclsid ? rclsid->Data1 : 0,
        rclsid ? rclsid->Data2 : 0,
        rclsid ? rclsid->Data3 : 0);
...
void InitWebBrowserHook() {
    Log("[webview2] InitWebBrowserHook: comecando\r\n");
    hOle32Module = LoadLib("ole32");
    hOleAut32Module = LoadLib("oleaut32");
    Log("[webview2] modulos: ole32=0x%p oleaut32=0x%p\r\n", hOle32Module, hOleAut32Module);
    ...
    Log("[webview2] hooks instalados: CoGetClassObject=0x%p CoCreateInstance=0x%p\r\n",
        pCoGetClassObject, pCoCreateInstance);
